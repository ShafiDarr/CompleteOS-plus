...
**Approved 2026-07-25**, after two rounds of review. Final decisions on top of the design below: field name is `reminder_enabled` (not `is_remindable`); Current Action keeps each commitment's own type icon as primary, with the reminder shown as a small secondary bell badge alongside it, never replacing it; `reminder_enabled = true` gives **no** automatic ordering boost — Current Action ordering stays governed entirely by the existing scheduling/status/priority/execution logic, and Reminder may affect presentation/alert visibility only, never silently override prioritization; `reminder_level` and `acknowledged_at` are both removed via migration (not deprecated in place); and this ADR's resolution of `reminder_level`/`acknowledged_at` is confirmed to narrow `MIGRATION_PLAN.md` Phase 2B's vestigial-column bucket down to the calendar-sync/verification columns only (`calendar_sync`, `calendar_event_id`, `calendar_synced_at`, `completion_synced`, `requires_verification`, `verification_status`). See "Final Decisions" below for the full resolution of each open question, and `MIGRATION_PLAN.md` Phase 2A.4 for the implementation-ready plan.
...
**A single boolean capability (`reminder_enabled`) on `tasks`, anchored to whichever scheduling field is currently the commitment's primary one, with no new timing field and no acknowledgment state of any kind (including inside `status`).**
...
2. **Minimum V1 behaviors required (item 2):**
   ...
   - It surfaces through the *existing* Current Action / Plan Today mechanisms — no new surfacing engine. **Final decision:** no special ordering treatment — a remindable task competes on exactly the same `priority_rank`/scheduling/status/execution logic as everything else; Reminder may affect presentation or alert visibility but must never silently override prioritization.
...
8. **How this differs from adjacent concepts (item 8):**
   ...
   - **Current Action:** the prioritization *engine* that decides what's next across every commitment. A remindable task is just one more input into that engine — it does not compete with or duplicate Current Action, and **does not get any ordering boost** (final decision, see status line above): ordering stays entirely governed by the existing `priority_rank`/scheduling/status/execution logic. Reminder is a presentation/alert-visibility signal only.
   - **Recurring occurrences:** explicitly out of scope for this ADR per instruction. Worth flagging directly: the 4-duplicate-row pattern in the live data looks like exactly the kind of manual workaround a real Recurring Templates/repeat decision (Phase 2B.2) might eventually remove — but that connection is noted here only, not resolved.
9. **Migration of the 4 live rows (item 9):** `task_type` changes from `'Reminder'` to `'Task'` for all 4 rows (nothing else distinguishes them from an ordinary task once the type is gone); set `reminder_enabled = true` on all 4 (preserves original intent); `due_at`, `status`, `priority` all carry over unchanged; `reminder_level`/`acknowledged_at` are already `NULL` on all 4, so their removal loses no data. Zero ambiguity, zero data loss — a straightforward 4-row `UPDATE`.
10. **Smallest V1 implementation (item 10):**
    - **Schema:** one new boolean column on `tasks`, `reminder_enabled`, default `false`; drop `'Reminder'` from the `task_type` check constraint; drop `reminder_level` and `acknowledged_at` columns entirely.
    - **FlutterFlow/`lib/`:** remove the `Reminder` entry from the `task_type` selector (can ride the same change as ADR-002's `Event` removal, since both touch the same dropdown); add a single toggle/checkbox ("Remind me") near the Deadline field in TaskForm, wired through EditorHost's existing save path exactly like any other boolean field (e.g. `is_active`) — no new save-flow branch needed; change the Systems Control Panel "Reminders" list filter from `task_type = 'Reminder'` to `reminder_enabled = true` (ordering by `due_at` unchanged); change Current Action's icon logic from *replacing* the type icon with a bell when `task_type == 'Reminder'` to *adding* a small secondary bell badge alongside the existing type icon when `reminder_enabled == true` — **final decision:** the commitment-type icon (Task/Habit/Routine/Bill/Appointment) always stays primary; the bell is a badge, never a replacement.
...
- Reminder stops being a first-class, independently listable "kind" of commitment. Anywhere the product wants "show me my Reminders," that becomes "tasks where `reminder_enabled` is set" instead of a `task_type` filter — a real filtering/UX change, not just a rename (scoped in item 10 above).
...
- `tasks.reminder_enabled` — new boolean column, default `false`.
- `tasks.task_type` — drop `'Reminder'` from the check constraint (5 remaining values, down from 6 after ADR-002).
...
- Supabase migration: add `reminder_enabled`; 4-row `UPDATE` (`task_type = 'Reminder'` → `'Task'`, `reminder_enabled = true`); drop `task_type` constraint's `'Reminder'` value; drop `reminder_level`/`acknowledged_at` columns. Can be sequenced together with ADR-002's Event-removal migration since both touch `task_type` and are low-risk/low-row-count.
- `schema.sql` refresh to match.
- `lib/`: TaskForm type selector, Systems Control Panel "Reminders" list filter, Current Action's icon logic (bell badge alongside the type icon, never replacing it) — all scoped in item 10 above. No Repository-layer redesign needed; this is additive-boolean-field work, not a new data-access pattern.
- Canonical docs updated as part of this approval: `DOMAIN_MODEL.md`, `DATABASE.md`, `GLOSSARY.md`, `V1_PRODUCT.md` (Commitment Types drop from six to five), `MIGRATION_PLAN.md` (this item is now Phase 2A.4, ready to implement; **Phase 2B's vestigial-column-set scope narrows** — `reminder_level`/`acknowledged_at` are resolved by this ADR and removed from that bucket, which is renumbered to Phase 2B.1 and now covers only the calendar-sync/verification columns, untouched by this ADR).
...
- Systems Control Panel: "Reminders" section becomes a capability-filtered list (`reminder_enabled = true`) instead of a type-filtered one; visually unchanged otherwise.
- Current Action: a small secondary bell badge appears alongside the commitment's existing type icon when `reminder_enabled = true`; the type icon (Task/Habit/Routine/Bill/Appointment) always stays primary and is never replaced.
...
### Final Decisions (Approved 2026-07-25)

1. **Field name:** `reminder_enabled` — not `is_remindable`. `is_remindable` reads as "eligible to support reminders"; `reminder_enabled` correctly communicates that the capability has actually been turned on for this commitment.
2. **Icon treatment in Current Action:** the commitment-type icon (Task/Habit/Routine/Bill/Appointment) stays primary always. When `reminder_enabled = true`, a small bell badge or secondary indicator is shown alongside it. The bell never replaces the Task/Habit/Routine/Bill/Appointment icon — Reminder is a capability, not the commitment's identity.
3. **Current Action ordering:** no automatic ordering boost for `reminder_enabled = true`. Ordering stays governed entirely by the established scheduling, status, priority, and execution logic. Reminder may affect presentation or alert visibility, but must never silently override prioritization.
4. **Destructive column drops:** confirmed — `reminder_level` and `acknowledged_at` are both removed through migration (not deprecated in place, not folded into `status`).
5. **Scope boundary with the vestigial-column bucket:** confirmed — this ADR resolves `reminder_level` and `acknowledged_at`. They are removed from `MIGRATION_PLAN.md`'s still-open vestigial-column question (renumbered Phase 2B.1), which now covers only `calendar_sync`, `calendar_event_id`, `calendar_synced_at`, `completion_synced`, `requires_verification`, and `verification_status`.
...
- **Resolved by this ADR's approval:** V1_PRODUCT.md, DOMAIN_MODEL.md, DATABASE.md, and GLOSSARY.md now state five Commitment Types (updated 2026-07-25, reflecting both this ADR and ADR-001's subsequent approval, which removed Reminder too).
...
**Canonical docs updated 2026-07-25 to match the three Approved ADRs (interim state, since superseded below):** `DOMAIN_MODEL.md`, `DATABASE.md`, `ARCHITECTURE.md`, `V1_PRODUCT.md`, `GLOSSARY.md`, `ROADMAP.md`, and `MIGRATION_PLAN.md` described Event as removed, `completed` as approved-for-removal, and `is_active` as approved-for-supersession-by-`lifecycle_state` (`tasks` only, Phase 1) — each clearly marked as approved target architecture, not yet implemented. `Reminder` remained untouched everywhere at that point, including as a live `task_type`, since ADR-001 had not yet been approved.

**ADR-001 Approved 2026-07-25, third pass — all four ADRs now Approved.** Final decisions: field name `reminder_enabled` (not `is_remindable`); Current Action keeps the commitment-type icon as primary, with the bell as a secondary badge only, never a replacement; no automatic ordering boost for `reminder_enabled = true` — ordering stays governed entirely by the existing scheduling/status/priority/execution logic; `reminder_level` and `acknowledged_at` both removed via migration; and this ADR's resolution of those two fields is confirmed to narrow `MIGRATION_PLAN.md`'s vestigial-column bucket (renumbered Phase 2B.1) down to the calendar-sync/verification columns only. See ADR-001's "Final Decisions" section above for the complete resolution of each point.

**Canonical docs updated 2026-07-25 to reflect all four Approved ADRs:** `DOMAIN_MODEL.md`, `DATABASE.md`, `GLOSSARY.md`, and `V1_PRODUCT.md` now describe five Commitment Types (Event and Reminder both removed as `task_type` values), with Reminder replaced by the universal `reminder_enabled` capability — each still marked **approved target architecture, not yet implemented**, per the current-vs-target discipline used throughout this document. The live database and app code remain unchanged; `task_type = 'Event'`, `task_type = 'Reminder'`, `tasks.completed`, `tasks.is_active`, `tasks.reminder_level`, and `tasks.acknowledged_at` all still exist and function exactly as before until the corresponding `MIGRATION_PLAN.md` phases are actually executed. `MIGRATION_PLAN.md` now lists Reminder as Phase 2A.4 (Approved, ready to implement) rather than Phase 2B.1, and its former vestigial-column-set item is renumbered Phase 2B.1 (Recurring Templates becomes 2B.2), scoped down to the calendar-sync/verification columns only.

**Status: all four ADRs (001–004) are Approved. No architecture decision remains open in this document.** Implementation may now proceed per `MIGRATION_PLAN.md`'s normal phase-by-phase process — Phase 2A (2A.1–2A.4) is fully decision-ready; Phase 2B (vestigial calendar-sync/verification columns, Recurring Templates scope) remains open and still requires Product Architect decisions before implementation, same as before.
