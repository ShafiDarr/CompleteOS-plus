...
- **Date/time of audit:** 2026-07-14 05:10 UTC (scoring corrections applied 2026-07-15 02:08 UTC — product owner clarified Task-type completion criteria and flagged the Current Action icon gap; self-challenge pass applied 2026-07-15 21:56 UTC — found a hardcoded fake score widget live on the dashboard and confirmed an entire unused verification/calendar-sync column set; **new FlutterFlow export verified merged 2026-07-21 21:39 UTC (commit `1c08b68`, merged into `develop` via PR #14) — re-verified against actual merged code, not just diffs, on 2026-07-21**; **live Supabase project queried directly 2026-07-24 — RLS, `profiles` trigger, `priority_rank`/`current_action_candidates`, and actual row data all verified server-side for the first time, see updated Blockers and Audit Confidence below**; **implementation regression reported by the Product Architect 2026-07-25 — see Blocker #8: Start Date/End Date wiring (`start_at`/`end_at`), credited below as working in commit `1c08b68`, was subsequently removed during unrelated EditorHost debugging and only `due_at` was restored; scoring and all "wired end-to-end" claims for Appointment/Event corrected accordingly**)
- **Audited commit:** `6c3d02e` (merge of PR #14, includes the audit docs from PR #13 and the new FlutterFlow export) — **note: the Start/End regression above happened after this commit, in local/uncommitted FlutterFlow debugging, so it is not visible as a diff against this commit; it is documented here on the Product Architect's direct report, not from re-reading the code**
...
- **Overall V1 completion: ≈51%** (down from ≈52% — Appointment/Event task-type support reverted from 50% to 25% each following the Start/End regression reported 2026-07-25; see Weighted Scoring Table and Blocker #8)
...
| Task-type support: Appointment | 1.67 | 25% | 0.42 | **[REGRESSED, reported 2026-07-25]** `start_at`/`end_at` wiring was verified working in commit `1c08b68` but was subsequently removed during unrelated EditorHost debugging; only `due_at` was restored afterward. Currently indistinguishable from any other type — no type-specific fields, DB mappings, create/edit wiring, display metadata, or execution behavior. Needs restoring, not merely extending — see MIGRATION_PLAN.md Phase 4.1. |
...
| Task-type support: Event | 1.67 | 25% | 0.42 | **[REGRESSED, reported 2026-07-25]** Same regression as Appointment — `start_at`/`end_at` wiring verified working in `1c08b68`, since removed; only `due_at` currently wired. See MIGRATION_PLAN.md Phase 4.1. |
...
| **Total** | **100** | — | **≈51%** (down from ≈52% — Appointment and Event task-type support each reverted 50%→25% after the Product Architect reported the Start/End wiring regression on 2026-07-25; the earlier ≈52% still reflected the now-reverted `1c08b68` wiring as current) | |
...
**Removed from this list 2026-07-25 (regression, not a documentation error):** "Appointment/Event `start_at`/`end_at` fields now have real end-to-end wiring" was true and verified as of commit `1c08b68`, but the Product Architect reports that wiring was subsequently removed during unrelated EditorHost debugging, and only `due_at` was restored afterward. See Blocker #8. Current production scheduling relies on `due_at` only, for every task type — Start Date/End Date are not currently wired for any type, not just Appointment/Event.
...
6. Habit, Routine, Bill, and Reminder task types remain completely unwired for type-specific fields. Appointment/Event got `start_at`/`end_at` in the `1c08b68` export, but **that wiring has since been reverted — see Blocker #8** — so as of 2026-07-25 all six non-base types are equally unwired, not just these four. Habit and Routine are the two types actually required by the V1 Definition of Done, and neither was ever touched by the Start/End work.
7. **[NEW, live data 2026-07-24]** `tasks.completed` (boolean) and `tasks.status` (text) are redundant and actively out of sync in production: 12 of the 24 live rows have `completed = false` while `status = 'Completed'` (zero rows show the opposite mismatch). The 3-value `status` field is clearly the one the UI actually reads/writes (consistent with the already-verified `Pending`/`In Progress`/`Completed` dropdown); `completed` looks like an orphaned FlutterFlow default nothing keeps updated. Not urgent by itself, but anything that ever reads `completed` (a filter, a metric, a future migration) will silently get the wrong answer for half the completed tasks in the DB today. Worth consolidating onto `status` alone when the schema is next touched, same treatment as the `calendar_sync`/verification column set in item 5. **Resolved as ADR-004 in `ARCHITECTURE_DECISIONS.md` (Approved 2026-07-25)** — `completed` will be dropped once the remaining ADRs clear review and canonical docs are updated.
8. **[REGRESSION, reported by the Product Architect 2026-07-25]** Start Date (`start_at`) and End Date (`end_at`) are **not currently wired**, for any `task_type`. Item 6 in this list and this audit's earlier scoring (Appointment/Event at 50%) reflected commit `1c08b68`'s wiring, which was genuinely working when verified 2026-07-21. That wiring was subsequently removed while debugging an unrelated issue (EditorHost failing to open TaskForm); only the Due Date implementation was restored afterward. **Current production baseline: `due_at` is the only scheduling field wired end-to-end, for every task type — this is implementation state, not a change to the approved Start/End/Deadline target architecture (ARCHITECTURE.md, 2026-07-22), which is unaffected.** Concretely this means: `DOMAIN_MODEL.md`'s Task entity and Scheduling Model section, and `MIGRATION_PLAN.md` Phase 4.1, both need to state Start/End as *not yet implemented* (again) rather than *implemented for Appointment/Event, needs generalizing* — Phase 4.1 is now a full restoration-plus-generalization, not an extension of working code. The Current Action view/ordering must not be assumed to depend on `start_at` until this is rebuilt and re-verified — it still orders by `due_at` today, which is now the *correct* current-state description, not a stale one.
...
1. **Restore, then generalize, Start/End Date & Time to all task types**, demoting `due_at` to an optional "Deadline" field in the same form. **Corrected 2026-07-25: this is no longer "extend the already-proven Appointment/Event pattern" — that pattern was reverted during unrelated debugging (Blocker #8) and needs restoring from scratch before it can be generalized to the other five types.** `due_at` remains the current production scheduling field for every type until this is rebuilt and verified.
2. **Update Current Action's ordering** from `priority_rank`/`due_at` to `priority_rank`/`start_at` (`execution_page_widget.dart:198`) — do this only after step 1 is restored *and verified*, not on the assumption `start_at` is already populated anywhere; it depends on `start_at` being reliably wired and populated across all types first.
